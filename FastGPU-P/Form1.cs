...
namespace FastGPU_P
{
    public partial class Form1 : MetroForm
    {
        ...
        {
            int value = (int)(Math.Round(allocationBar.Value / 5.0) * 5);
            allocationBar.Value = value;
            this.allocPercent.Text = value.ToString() + "%";
        }
        private static string GetGPUInstance(string name)
        {
            string _scr = (@"
